...
async function seedSongs() {
	...
	for (let file of files.filter((file) => path.extname(file).toLowerCase() === '.mp3')) {
		...
		let existingSong = await db.select().from(songs).where(eq(songs.audioUrl, songData.audioUrl)).limit(1)
...
async function seedPlaylists() {
	...
	for (let name of playlistNames) {
		...
		const existingPlaylist = await db.select().from(playlists).where(eq(playlists.name, name)).limit(1)

...
function TrackRowPreview({
	...
	let { currentTrack, playTrack, togglePlayPause, isPlaying, setActivePanel, handleKeyNavigation } = usePlayback()
	...
	return (
		...
		>
			...
			<td className="py-[2px] px-2">
				<div className="flex items-center">
					...
					<div className="font-medium truncate max-w-[180px] sm:max-w-[200px] text-[#d1d5db]">
						...
						<span className="sm:hidden text-gray-400 ml-1">• {highlightText(track.artist, query)}</span>
					</div>
				</div>
			</td>
			...
			<td className="py-[2px] px-2 hidden md:table-cell text-[#d1d5db]">{highlightText(track.album!, query)}</td>
			<td className="py-[2px] px-2 tabular-nums text-[#d1d5db]">{formatDuration(track.duration)}</td>
			<td className="py-[2px] px-2">
...
export function TrackTablePreview({ playlist, query }: { playlist: PlaylistWithSongs; query?: string }) {

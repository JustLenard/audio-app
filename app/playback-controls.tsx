...
export function PlaybackButtons() {
	let { isPlaying, togglePlayPause, playPreviousTrack, playNextTrack, currentTrack } = usePlayback()
	...
	return (
		<div className="flex items-center space-x-2">
			...
			<Button variant="ghost" size="icon" className="h-8 w-8" onClick={togglePlayPause} disabled={!currentTrack}>
				{isPlaying ? <Pause className="w-5 h-5 stroke-[1.5]" /> : <Play className="w-5 h-5 stroke-[1.5]" />}
			</Button>
			<Button variant="ghost" size="icon" className="h-8 w-8" onClick={playNextTrack} disabled={!currentTrack}>
...
export function PlaybackControls() {
	let { currentTrack, audioRef, setCurrentTime, setDuration, playPreviousTrack, playNextTrack, togglePlayPause } =
		usePlayback()
	...
	console.log('This is currentTrack', currentTrack)

		...
		<li className="group relative">
			<Link
				...
				className={`block py-1 px-4 cursor-pointer hover:bg-[#1A1A1A] text-[#d1d5db] focus:outline-none focus:ring-[0.5px] focus:ring-gray-400 ${
					pathname === `/music/p/${playlist.id}` ? 'bg-[#1A1A1A]' : ''
				}`}
				tabIndex={0}
			>
				{playlist.name}
...
export function OptimisticPlaylists() {
	...
	async function addPlaylistAction() {
		...
		router.prefetch(`/music/p/${newPlaylistId}`)
		router.push(`/music/p/${newPlaylistId}`)
			...
			<div className="m-4">
				...
				<div className="flex justify-between items-center mb-4">
					...
					<form action={addPlaylistAction}>
						<Button disabled={isProduction} variant="ghost" size="icon" className="h-5 w-5" type="submit">
